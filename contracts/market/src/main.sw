...
// # Arguments
    ...
    fn pause_collateral_asset(asset_id: AssetId) {
        ...
        only_owner();
        let mut
 configuration = storage.collateral_configurations.get(asset_id).read();
...
// This function retrieves all collateral asset configurations in the market.
    ...
    fn get_collateral_configurations() -> Vec<CollateralConfiguration> {
        ...
        let mut index = 0;
        let len
 = storage.collateral_configurations_keys.len();
        while index
 < len {
...
: AssetId = msg_asset_id();
 ...
 = storage.totals_collateral.get(asset_id).try_read().unwrap_or(0) + amount;
        require(
...
// This function ensures that the withdrawal adheres to the market's rules and checks for collateralization.
    ...
    fn withdraw_collateral(
        ...
        require(
!storage.pause_config.withdraw_paused.read(), Error::Paused);
...
// This function allows users to supply base assets to the market, updating their balance and the market's total supply.
 ...
 = msg_sender().unwrap();
        ...
        let user_balance
 = present_value(user_principal) + I256::try_from(u256::from(amount)).unwrap();
        ...
        let mut
 market_basic = storage.market_basic.read();
...
// This function calculates the amount of base asset a user can borrow based on their collateral and current borrow position.
 ...
 < len {
            let collateral_configuration = storage.collateral_configurations.get(storage.collateral_configurations_keys.get(index).unwrap().read()).read();
            let balance
: u256 = storage.user_collateral.get((account, collateral_configuration.asset_id)).try_read().unwrap_or(0).into();
            if balance
 == 0 {
                index += 1;
                continue;
 ...
 = get_price_internal(
            ...
            borrow_limit +=
 amount * price * base_asset_scale / collateral_scale / price_scale; // decimals: base_token_decimals
            index += 1;
        };
...
// This function absorbs a list of underwater accounts onto the protocol balance sheet.
    ...
    fn absorb(accounts: Vec<Identity>, price_data_update: PriceDataUpdate) {
        ...
        require(
!storage.pause_config.absorb_paused.read(), Error::Paused);
        // Accrue interest
        accrue_internal(
);
        // Update price data
...
// This function allows buying collateral from the protocol. Prices are not updated here as the caller is expected to update them in the same transaction using a multicall handler.
    ...
    fn buy_collateral(asset_id: AssetId, min_amount: u64, recipient: Identity) {
        ...
        require(
!storage.pause_config.buy_paused.read(), Error::Paused);
        ...
        let reserves
 = get_reserves_internal() - I256::try_from(u256::from(payment_amount)).unwrap();
 ...
 = quote_collateral_internal(asset_id, payment_amount);
...
// This function calculates the base asset value for selling a collateral asset.
 ...
 = get_price_internal(
        ...
        let collateral_scale = u256::from(10_u64).pow(collateral_configuration.decimals);
        let base_asset_scale = u256::from(10_u64).pow(market_configuration.base_token_decimals);
        let asset_discounted
 = asset_price_discounted * collateral_amount.into() / asset_price_scale; // decimals: collateral_asset.decimals
...
fn is_borrow_collateralized(account: Identity) -> bool {
    ...
    let present
 = present_value(principal); // decimals: base_token_decimals
 ...
 < len {
        let collateral_configuration = storage.collateral_configurations.get(storage.collateral_configurations_keys.get(index).unwrap().read()).read();
        let balance
: u256 = storage.user_collateral.get((account, collateral_configuration.asset_id)).try_read().unwrap_or(0).into(); // decimals: collateral_configuration.decimals
        if balance == 0 {
            index += 1;
            continue;
        }
 ...
 = get_price_internal(
        ...
        let amount
 = balance * price / price_scale; // decimals: collateral_configuration.decimals
...
fn is_liquidatable_internal(account: Identity, present: I256) -> bool {
    ...
    let present
: u256 = present.wrapping_neg().try_into().unwrap(); // decimals: base_token_decimals
...
: u256 = timestamp().into();
 ...
 > 0 {
        ...
        let base_scale
 = u256::from(10_u64).pow(storage.market_configuration.read().base_token_decimals);
...
fn update_base_principal(account: Identity, basic: UserBasic, principal_new: I256) {
 ...
 = basic.principal;
    ...
    let accrual_descale_factor
: u256 = u256::from(10_u64).pow(market_configuration.base_token_decimals) / BASE_ACCRUAL_SCALE;
...
fn quote_collateral_internal(asset_id: AssetId, base_amount: u64) -> u64 {
 ...
 = get_price_internal(
    ...
    let value
 = base_price * base_amount.into() / base_scale; // decimals: base_price.exponent
...
fn absorb_internal(account: Identity) {
    ...
    require(
        is_liquidatable_internal(account, old_balance),
        Error::NotLiquidatable,
    );
    let mut
 delta_value: u256 = 0; // decimals: 18
    let market_configuration = storage.market_configuration.read();
    let caller
 = msg_sender().unwrap();
    // Only used for logging event
    let mut
 total_value: u256 = 0; // decimals: 18
    let mut index = 0;
    let len = storage.collateral_configurations_keys.len();
    while index
 < len {
        ...
        let total_collateral
 = storage.totals_collateral.get(collateral_configuration.asset_id).try_read().unwrap_or(0);
        storage
            .totals_collateral
            .insert(
                collateral_configuration
 ...
 = get_price_internal(
        ...
        delta_value +=
 price * seize_amount.into() * collateral_configuration.liquidation_penalty / collateral_scale / price_scale; // decimals: 18
        index += 1;
        // Total value of seized collateral with liquidation penalty
        total_value +=
 price * seize_amount.into() * FACTOR_SCALE_18 / collateral_scale / price_scale; // decimals: 18
 ...
 = get_price_internal(
    ...
    let delta_balance
 = delta_value * base_price_scale * base_scale / base_price / FACTOR_SCALE_18; // decimals: base_token_decimals
 ...
 market_basic = storage.market_basic.read();
    ...
    log(MarketBasicEvent { market_basic });
    let total_base
 = total_value * base_price_scale * base_scale / base_price / FACTOR_SCALE_18; // decimals: base_token_decimals

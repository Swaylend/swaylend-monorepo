...
const TokenRowDisplay: React.FC<IProps> = ({ token }) => {
  ...
  const supplyCap =
    collateralConfigurations == null
      ? BN.ZERO
      : new BN(collateralConfigurations[token.assetId].supply_cap.toString());
  const collateralReserve =
    totalCollateralInfo == null ? BN.ZERO : totalCollateralInfo[token.assetId];
  const protocolBalance =
    collateralBalances != null ? collateralBalances[token.assetId] : BN.ZERO;
        ...
        <Row alignItems="center">
          ...
          <Column>
            ...
            <Text size="small" weight={600} type="secondary">
              {isConnected
                ? `${token.symbol} • ${balance?.format({ precision: 4, units: token.decimals })} in wallet`
                : `${token.symbol}`}
            </Text>
          </Column>
        </Row>
...
export default TokenRowDisplay;

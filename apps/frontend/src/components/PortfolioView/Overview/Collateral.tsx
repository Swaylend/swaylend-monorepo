...
const CollateralTableRow = ({
    market, assetId, value, amount, apy, liquidationRisk
}: TableRowProps) => {
    ...
    return (
        <TableRow>
            ...
            <TableCell className={`font-semibold bg-card ${liquidationRisk > 80 && 'text-red-500'} ${liquidationRisk > 60 && liquidationRisk <= 80 && 'text-yellow-500'} ${liquidationRisk <= 60 && 'text-primary'}`}>
                {liquidationRisk}%
            </TableCell>
            <TableCell className="text-white font-semibold bg-card">
                <div className="flex gap-x-1 items-center text-primary">
                    ...
                    <div>
                        {' '}
                        {apy?.borrowRewardApr.times(100).toFixed(2)}%
                    </div>
                </div>
            </TableCell>
            <TableCell className="">
...
export const Collateral = () => {
    ...
    const suppliedCollateralsUSDT = useMemo(() => {
        if (!priceDataUSDT || !userCollateralAssetsUSDT || !colateralConfigurationsUSDT) return [];
        ...
        return assetIDs.filter((assetId) => {
            ...
            const amount = formatUnits(userCollateralAssetsUSDT[assetId], colateralConfigurationsUSDT[assetId].decimals);
            const market = 'USDT';
            const value = getFormattedPrice(priceDataUSDT.prices[assetId].times(amount));
    ...
    const suppliedCollateralsUSDC = useMemo(() => {
        if (!priceDataUSDC || !userCollateralAssetsUSDC || !colateralConfigurationsUSDC) return [];
        ...
        return assetIDs.filter((assetId) => {
            ...
            const amount = formatUnits(userCollateralAssetsUSDC[assetId], colateralConfigurationsUSDC[assetId].decimals);
            const market = 'USDC';
            const value = getFormattedPrice(priceDataUSDC.prices[assetId].times(amount));
            return { market, assetId, value, amount }
        });
    ...
    return (
        <Table className="max-lg:hidden mt-12">
            ...
            <TableBody>
                {suppliedCollateralsUSDT.length === 0 && suppliedCollateralsUSDC.length === 0 && <TableRow>
                    <TableCell colSpan={8}>

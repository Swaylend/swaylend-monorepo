...
export const Borrow = () => {
  const rows = Object.keys(appConfig.client.v2.markets).map((market) => (
    <BorrowRow key={market} market={market} />
  ));
    ...
    <TableRow key={market}>
      ...
      <TableCell>
        <span className="font-medium text-lavender">
          {getFormattedPrice(borrowedPrice)}
        </span>{' '}
        {borrowed.toFixed(2)}
      </TableCell>
      <TableCell>
      ...
      <TableCell
        className={cn(
          ...
          currentCollateralUtilization > 80 && 'text-red-500',
          currentCollateralUtilization > 60 &&
            currentCollateralUtilization <= 80 &&
            'text-yellow-500',
          currentCollateralUtilization <= 60 && 'text-primary'
        )}
      >

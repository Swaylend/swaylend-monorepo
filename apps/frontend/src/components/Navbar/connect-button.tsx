...
export const ConnectButton = () => {
  ...
  if (isConnected && account) {
    return (
      <Popover>
        <PopoverTrigger>
          <div className="px-4 py-2 rounded-full max-xl:hidden flex gap-x-2 items-center bg-secondary text-moon font-semibold cursor-pointer">
            {`${account?.slice(0, 6)}...${account?.slice(-4)}`}

            <ChevronDown className="h-4 w-4" />
          </div>
          <div className="px-4 py-2 rounded-full xl:hidden flex gap-x-2 items-center bg-secondary text-moon font-semibold cursor-pointer">

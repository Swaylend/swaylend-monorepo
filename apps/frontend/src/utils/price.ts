...
export const getFormattedPrice = (number: BigNumber): string => {
  ...
  if (number.gte(1_000_000_000)) {
    return `$ ${number.div(1_000_000_000).toFixed(2, BigNumber.ROUND_FLOOR)}B`;
  }
  if (number.gte(1_000_000)) {
    return `$ ${number.div(1_000_000).toFixed(2, BigNumber.ROUND_FLOOR)}M`;
  }
  if (number.gte(1000)) {
